...
MODEL = "gpt-4.1-mini"  # Prioritize stability for judge. Upgrade to GPT-4.1 if necessary.
TOPK = 100
...
class CrossEncoderReranker:
    ...
    def rerank(
        ...
        # 3) Score Inference Using Cross-Encoder
...
def _should_accept_negation(flag_name: str, flag_obj: Dict[str, Any], full_text: str) -> bool:
    ...
    # Relative evaluation/not primary is not a “negation,” so it is omitted.
    if ev and RELATIVE_NOT_MAIN_PAT.search(ev):
        return False

    # Do not adopt a 0 where there is no strong evidence against it (conservative)
    if ev and not STRONG_NEG_PAT.search(ev):
        return False

    # If not explicitly specified, do not adopt 0 (to prevent accidental use of 0).
...
def get_simirality_ranking(query_text):
    ...
    query_flags = postprocess_query_flags(query_text,query_flags) #Readjustment after extraction
